...
public class RepositoryBase<T> : IRepositoryBase<T> where T : EntityBase
{
    ...
    public Pagination<T> PaginatedFind(PaginationFilter filter, bool trackChanges = false)
    {
        ...
        return PaginateItems(filter, query);
    }
    ...
    public async Task<bool> ExistAsync(Guid id)
    {
        return await _context.Set<T>()
            .AnyAsync(x => new Guid(x.GetType().GetProperty("Id")!.ToString() ?? "") == id && !x.IsDeleted);
    }
    ...
    private static Pagination<T> PaginateItems(PaginationFilter filter, IQueryable<T> query)
    {
